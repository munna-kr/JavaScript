...
const numbers = [5, 3, 9, 1, 5, 7];
// reverse -> [7,5,1,9,3,5]
// sort ascending -> [1,3,5,5,7,9]
// remove duplicates from ["apple","banana","apple"] -> ["apple","banana"] (order may vary)
// sum using reduce -> 30
```
---
...
```js
const people = { John: 25, Alice: 30, Eve: 15 };
// max age -> { name: 'Alice', age: 30 }
// count >=18 -> 2
// Object.entries -> [['John',25], ['Alice',30], ['Eve',15]] (order may vary)
```
---
...
```js
const users = new Map([
  ['John',25], ['Alice',30], ['Eve',15]
]);
// get 'Alice' -> 30
// size -> 3
// filter age>=18 -> Map { 'John'=>25, 'Alice'=>30 }
// firstKey -> 'John'
```
---
...
```js
// BST with values [10,5,15,2,7]
// inorder -> [2,5,7,10,15]
// find min -> 2
// height -> 3 (depending on definition)
```
---
...
// BFS from A -> ['A','B','C','D'] (one valid order)
// shortest path A->D -> ['A','B','D'] or ['A','C','D']
```
---
...
// search('cat') -> true
// startsWith('ca') -> true
// autocomplete('ca') -> ['car','cat','cart'] (order may vary)
```
---
...
// groupBy user -> { A: [{...},{...}], B: [{...}] }
// user with highest total -> 'A' (total 250)
// frequency counter -> Map { 'A' => 2, 'B' => 1 }
```
---
